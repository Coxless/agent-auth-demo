...
上表はロールのみで決まる静的な認可。実際にはこれに加えて §6.5 の動的認可 2 種が重なる。
...
- エンティティ:
  ...
  - `Record`: 認可対象のリソース。属性 `sensitivity: String`（値は `normal | confidential`）
- アクション: `readRecord`, `writeRecord`, `deleteRecord`（適用 principal=`User`, resource=`Record`）
  - `deleteRecord` の `context` は `targetCount: Long`（1 回の呼び出しで対象になるレコード数）を持つ。

### 6.3 Cedar ポリシー

Cedar は「明示的に permit されない限り deny」。forbid は permit より常に優先される。

```cedar
// 読み取り: editor / admin は常に許可
permit(principal, action == Action::"readRecord", resource)
when { principal.role == "editor" || principal.role == "admin" };
// viewer は confidential でないレコードのみ（ABAC: resource.sensitivity）
permit(principal, action == Action::"readRecord", resource)
...
// editor は confidential でないレコードのみ（ABAC）
permit(principal, action == Action::"writeRecord", resource)
...
// 一括削除ガード: 1 回の呼び出しで 2 件以上を対象にすると admin でも拒否（context ベース）
...
このデモの Cedar 判定は role だけの静的な RBAC ではなく、リクエストごとの状況で結果が変わる動的な判定を含む。

1. **ABAC（リソース属性ベース）**: 同じ role・同じ action でも、対象レコードの `sensitivity` 属性によって
   allow/deny が変わる。`r2` は `confidential` として登録済みで、viewer は read 不可、editor は write 不可
   （通常のレコードなら可能）。判定材料が「principal の静的な role」だけでなく「resource の実行時属性」に
   広がっている点が動的認可の本質。
2. **コンテキストベースの一括操作ガード**: `deleteRecord` は `context.targetCount`（1 回の呼び出しで対象に
   なるレコード数）を見て、2 件以上なら role に関係なく forbid する。同じ principal・同じ action・同じ
   resource の組でも、リクエストの内容（対象件数）によって結果が変わる。誤操作や暴走したエージェントに
   よる一括破壊を防ぐ「爆発半径の制限」の一例。
...
|---|---|---|---|
| `readRecord` | `{ id: string }` | viewer 以上（confidential は editor 以上） | レコード取得 |
| `writeRecord` | `{ id: string, data: object }` | editor 以上（confidential は admin のみ） | レコード作成/更新 |
| `deleteRecord` | `{ id: string }` または `{ ids: string[] }` | admin（1 回に 2 件以上は常に拒否） | レコード削除 |
...
| 11 | carol (viewer) | `readRecord` on `r2`（confidential） | **deny / 403**（通常レコードは allow） |
| 12 | bob (editor) | `writeRecord` on `r2`（confidential） | **deny / 403**（通常レコードは allow） |
| 13 | alice (admin) | `deleteRecord` with `ids: ["r1","r2"]` | **deny / 403**（admin でも 2 件以上は拒否） |
